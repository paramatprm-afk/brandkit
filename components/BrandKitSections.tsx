...
export function BrandKitSections({
  result,
  initialLogos,
  onLogosChange,
}: {
  result: BrandKitResult;
  /** Pre-generated logo images (e.g. from a saved brand). Missing/null entries are generated on mount. */
  initialLogos?: LogoImage[] | null;
  /** Called whenever the logo set settles, so a parent can include it when saving. */
  onLogosChange?: (logos: LogoImage[]) => void;
}) {
  return (
    <>
      ...
      <Section title="แท็กไลน์" subtitle="Tagline">
        <div className="rounded-2xl border border-orange-200 bg-orange-50 p-8 text-center">
          <p className="font-heading text-2xl font-semibold text-stone-900">{result.tagline.th}</p>
...
function LogoConceptsSection({
  ...
  concepts: string[];
  initialLogos?: LogoImage[] | null;
  onChange?: (logos: LogoImage[]) => void;
}) {
  const [logos, setLogos] = useState<LogoState[]>(() =>
    concepts.map((_, i) => {
      const preloaded = initialLogos?.[i];
      return preloaded ? { status: "done", imageDataUrl: preloaded } : { status: "loading" };
    }),
  );

  // Generate only the concepts that don't already have an image (fresh
  // generation: none do; reopening a saved brand: only the missing ones do).
  useEffect(() => {
    const controllers = concepts.map(() => new AbortController());

    async function generate(index: number, signal: AbortSignal) {
    ...
    concepts.forEach((_, i) => {
      if (initialLogos?.[i]) return;
      generate(i, controllers[i].signal);
    });
  ...
  useEffect(() => {
    onChange?.(logos.map((l) => (l.status === "done" ? l.imageDataUrl : null)));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [logos]);
  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
      {concepts.map((concept, i) => (
        <LogoCard key={i} index={i} concept={concept} state={logos[i]} onRetry={() => retry(i)} />
      ))}
    </div>
  );
}
